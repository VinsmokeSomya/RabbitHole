...
from rabbithole.a2a.types import (
    ...
    TaskQueryParams,
    AgentCard,
    TaskStatusUpdateEvent,  # For type hint
)
...
async def cli(
    ...
    if use_push_notifications:
        if not card_obj.capabilities.pushNotifications:
            print(
                "Agent does not support push notifications. Ignoring --use_push_notifications."
            )
            use_push_notifications = False
        elif not notification_receiver_host or not notification_receiver_port:
            print(
                "Push notification receiver host or port is missing. Ignoring --use_push_notifications."
            )
            use_push_notifications = False
        else:
            ...
            agent_server_base_url = (
                card_obj.url
            )  # This is the server's endpoint from its card
            ...
            server_jwks_url = urljoin(
                agent_server_base_url_for_join, ".well-known/jwks.json"
            )

            print(
                f"Attempting to load A2A Server's JWKS from: {server_jwks_url} (for verifying received push notifications)"
            )
            ...
            try:
                ...
                print(
                    f"JWKS loading for PushNotificationReceiverAuth from {server_jwks_url} needs actual implementation."
                )

            except Exception as e:
                print(
                    f"Failed to load A2A Server's JWKS: {e}. Push notifications might not be authenticated correctly."
                )
...
async def completeTask(
    ...
    if (
        use_push_notifications
        and notification_receiver_host
        and notification_receiver_port
    ):
        ...
        task_send_params.pushNotification = PushNotificationConfig(
            ...
            authentication=AuthenticationInfo(
                schemes=["bearer"]
            ),  # schemes is a list of strings
        )

    taskResult: Union[GetTaskResponse, SendTaskResponse, None] = None

    actual_streaming_to_use = streaming_supported
    if streaming_supported and (
        not hasattr(card.capabilities, "streamingHttpV1alpha")
        or card.capabilities.streamingHttpV1alpha is False
    ):
        print(
            "Agent supports general streaming but not streamingHttpV1alpha. Falling back to non-streaming."
        )
    ...
    if actual_streaming_to_use:
        try:
            ...
            async for result_item in response_stream:
                print(
                    f"stream event => {result_item.model_dump_json(exclude_none=True)}"
                )
                if (
                    result_item.result
                    and isinstance(result_item.result, TaskStatusUpdateEvent)
                    and result_item.result.final
                ):
                    pass
    ...
    if taskResult and taskResult.result and taskResult.result.status:
        ...
        print(
            f"Task {taskId} did not return a valid result or status. Ending interaction."
        )
        return False
