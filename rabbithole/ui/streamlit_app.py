...
def run_streamlit_app():
    ...
    with st.sidebar:
        ...
        st.session_state.agent_url = st.text_input(
            "Agent Server URL", value=st.session_state.agent_url
        )
    ...
    for author, message_content in st.session_state.conversation:
        with st.chat_message(author):
            if isinstance(message_content, str):
                ...
                isinstance(message_content, dict) and "url" in message_content
            ...
            with st.spinner("Thinking..."):
                try:
                    if not st.session_state.agent_url:
                        st.error(
                            "Agent Server URL is not set. Please configure it in the sidebar."
                        )
                        st.stop()
                    ...
                    user_message = Message(
                        role="user", parts=[TextPart(text=user_prompt)]
                    )
                    task_send_params = TaskSendParams(
                        id=f"streamlit-ui-{uuid4().hex}", message=user_message
                    )
                        ...
                        if returned_task.status and returned_task.status.message:
                            ...
                            for artifact_item in returned_task.artifacts:
                                ...
                                if found_artifact_content:
                                    break
                            if (
                                not found_artifact_content and returned_task.status
                            ):  # If artifacts existed but no text content found
                            ...
                            agent_response_display = (
                                f"Task status: {returned_task.status.state.value}"
                            )
                    ...
                    # Or if send_task_response was None (should not happen with current try/except but good for robustness)
                        ...
                        agent_response_display = (
                            "Failed to get any response from server."
                        )

                    st.session_state.conversation.append(
                        ("assistant", agent_response_display)
                    )
                    ...
                    error_message = (
                        f"HTTP error occurred: {http_err} - {http_err.request.url}"
                    )
                    try:
