...
def list_models():
    ...
    try:
        ...
        if not api_key:
            ...
            logging.error("API key is not set in the script. Please edit the HARDCODED_API_KEY variable.")
            return
        
        # Check if the key is the placeholder
        if api_key == "GEMINI_API_KEY" and api_key.startswith("AIzaSy") and len(api_key) < 40:
            ...
            logging.warning("The hardcoded API key looks like a placeholder or is invalid. Please replace HARDCODED_API_KEY with your actual Gemini API key in the script.")
        ...
        if model_count == 0:
            logging.warning("No models were returned. This might indicate an issue with the API key (e.g., permissions) or the API service.")
        else:
            logging.info(f"Found {model_count} models.")

    except Exception as e:
        logging.error(f"An error occurred: {e}")
        logging.error("Ensure your API_KEY is valid and the Google Generative AI API is enabled for your project.")
